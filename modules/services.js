...
class ServiceManager {
    ...
    async getServiceStatus(serviceName) {
        try {
            const { stdout } = await execPromise(`sudo systemctl status ${serviceName} --no-pager -n 5`);
    ...
    async getServiceLogs(serviceName, lines = 20) {
        try {
            const { stdout } = await execPromise(`sudo journalctl -u ${serviceName} -n ${lines} --no-pager`);
            return stdout;
        } catch (error) {
